...
for condition = 1:4
    ...
    for subIDX = 1:length(visTapSubs)
        ...
        compSpectra = 10.^(compSpectra/10); % convert to amplitude

        [chanSpectra,freqs] = spectopo(EEG.data, EEG.pnts, EEG.srate,'winsize', EEG.pnts,'plot','off');
        
        chanSpectra = 10.^(chanSpectra/10); % convert to amplitude
        ...
        data.LMuCompRawSpecta(subIDX,:) = compSpectra(data.subj(subIDX).MuComps.left,startIDX:endIDX);
        ...
        data.RMuCompRawSpecta(subIDX,:) = compSpectra(data.subj(subIDX).MuComps.right,startIDX:endIDX);
        ...
        % get all raw Chan Spectra
        data.ChanAllRawSpectra(subIDX,:,:) = chanSpectra(:,startIDX:endIDX);
