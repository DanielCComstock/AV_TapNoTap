...
saveMotorIndPath = '/Users/danielcomstock/Documents/Data/AV_TapNoTap/Data/08_Comp_TF_Plots/MotorCompInd/ITC/RightMotor/';
...
for conditionIDX = 1:4
    ...
    for subIDX = 1:length(MotorClusters)
        ...
        rmc = MotorClusters(subIDX).rm(1);
        
        itc = eTF_dataW.Data(rmc).ITC;
        times = eTF_dataW.times;
        freqs = eTF_dataW.freqs;
        
        MotorITC_Avg.data(subIDX).itc = itc;
        ...
        clear itc ITCMinMax ITCMin ITCMax 
    ...
    for subIDX = 1:length(MotorClusters)
        itc(:,:,subIDX)=MotorITC_Avg.data(subIDX).itc;
    end
    
    % Compute absolute highest itc values in the Average itc for symetrical scalling
    avgITC = mean(itc,3); 
    ITCmax = [max(max(avgITC))];
    ITCmin = [min(min(avgITC))];
    % convert absolute highest itc values for symetrical scale
    ITCMinMax = [ITCmin ITCmax]; 

    %% Plot ITC Ind
    
    saveName = [conMod(1:end-4) '_Right_Motor'];
    titleName = strrep(saveName, '_', ' ');


    figure; 
    imagesc(times, freqs, avgITC, ITCMinMax); 
    % sets scaling direction
    set(gca, 'ydir', 'normal'); 
    % set font
    set(gca, 'FontSize', 12, 'FontWeight', 'bold');
    ...
    % save as .fig file
    ITCfigfullsave = [saveMotorAvgPath '/fig/' saveName];
    savefig(ITCfigfullsave);        
    % save as .png file
    ITCfigfullsave = [saveMotorAvgPath '/png/'  saveName];
    saveas(gcf,ITCfigfullsave,'png');        
    % close figure
    close;
    ...
    fullDataSave = [saveMotorAvgPath titleName];
    save(fullDataSave, 'MotorITC_Avg');

    clear itc ITCMinMax ITCMin ITCMax MotorITC_Avg avgITC
end
