figpath = 'H:\Data\AV_TapNoTap\06_Figures\Channel_Figs\';
figDataPath='H:\Data\AV_TapNoTap\06_Figures\Channel_Figs\FigData\';
chans = ['FZ'; 'CZ'; 'PZ'; 'OZ'; 'C4'; 'C3'];
chan = cellstr(chans);
...
for i = 4:7
    ...
    for m = 1:6
        ...
        [STUDY itc times freqs ] = std_erspplot(STUDY,ALLEEG,'channels',{tempchan},'datatype', 'itc','plotmode','none'); % ERSP plot for cluster 13, and saves the ersp, times, and freqs output as variables
        % Save Normal ERSP
        ftitle = ['ITC ' tempname ' - ' tempchan];
       
%         [STUDY itc times freqs ] = std_erspplot(STUDY,ALLEEG,'channels',{'FZ'},'datatype', 'itc','plotmode','none'); % ERSP plot for cluster 13, and saves the ersp, times, and freqs output as variables
%          % Save Normal ERSP
%          ftitle = ['ITC Aud Control - FZ'];
%         
        avgersp = mean(itc{1},4); % average ERSP for all subjects
        avgersp = abs(avgersp);
